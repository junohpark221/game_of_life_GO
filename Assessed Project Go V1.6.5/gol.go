...
func merger(p golParams, world [][]byte, chans [maxthreads]chan [16]byte, remains int) {
	for n := 0; n < realworker; n++ {
		if n < remains {
			for y := 0; y <= (p.imageHeight / realworker); y++ {
				for x := 0; x < p.imageWidth/16; x++ {
					section := <-chans[n]
					for i := 0; i < 16; i++ {
						world[(n*(p.imageHeight/realworker+1))+y][16*x+i] = section[i]
					}
				}
			}
		} else {
			for y := 0; y < (p.imageHeight / realworker); y++ {
				for x := 0; x < p.imageWidth/16; x++ {
					section := <-chans[n]
					for i := 0; i < 16; i++ {
						world[(n*(p.imageHeight/realworker)+remains)+y][16*x+i] = section[i]
					}
				}
			}
		}
...
func importer(worldslice [][]byte, p golParams, remains int, localtotalalive int, io chan [16]byte, z int) int {
	localtotalalive = 0
	if z < remains {
		for y := 0; y < ((p.imageHeight / realworker) + 3); y++ {
			for x := 0; x < p.imageWidth/16; x++ {
				section := <-io
				for i := 0; i < 16; i++ {
		...
		for y := 0; y < ((p.imageHeight / realworker) + 2); y++ {
			for x := 0; x < p.imageWidth/16; x++ {
				section := <-io
				for i := 0; i < 16; i++ {
					worldslice[y][16*x+i] = section[i]
...
func gollogic(worldslice [][]byte, worldupdator [][]byte, p golParams, remains int, z int, localtotalalive int) int {
	if z < remains {
		for y := 1; y < ((p.imageHeight / realworker) + 2); y++ {
			for x := 0; x < p.imageWidth; x++ {
		...
		for y := 1; y < ((p.imageHeight / realworker) + 1); y++ {
			for x := 0; x < p.imageWidth; x++ {
				// Sum the number of adjacent alive cells wrapping around at the edges.
				alive := 0
				for i := -1; i < 2; i++ {
...
func sender(p golParams, worldslice [][]byte, io chan [16]byte, remains int, z int) {
	if z < remains {
		for y := 1; y < ((p.imageHeight / realworker) + 2); y++ {
			for x := 0; x < p.imageWidth/16; x++ {
				var section [16]byte
...
func haloex(p golParams, worldslice [][]byte, io chan [16]byte, halosaversup [maxthreads]chan [16]byte, halosaversdown [maxthreads]chan [16]byte, remains int, z int) {
	for x := 0; x < p.imageWidth/16; x++ {
		var section [16]byte
		for k := 0; k < 16; k++ {
			section[k] = worldslice[1][x*16+k]
		}
		halosaversdown[modulo(z-1, realworker)] <- section
	}
	for x := 0; x < p.imageWidth/16; x++ {
		section := <-halosaversdown[z]
		for i := 0; i < 16; i++ {
			if z < remains {
				worldslice[(p.imageHeight/realworker)+2][16*x+i] = section[i]
			} else {
				worldslice[(p.imageHeight/realworker)+1][16*x+i] = section[i]
			}
		}
	}
	...
	if z < remains {
		for x := 0; x < p.imageWidth/16; x++ {
			var section [16]byte
			for k := 0; k < 16; k++ {
				section[k] = worldslice[(p.imageHeight/realworker)+1][x*16+k]
			}
			halosaversup[modulo(z+1, realworker)] <- section
		}
	} else {
		for x := 0; x < p.imageWidth/16; x++ {
			var section [16]byte
			for k := 0; k < 16; k++ {
				section[k] = worldslice[(p.imageHeight / realworker)][x*16+k]
			}
			halosaversup[modulo(z+1, realworker)] <- section
		}
	}
		...
		if n < remains {
			for y := -1; y <= rows+1; y++ {
				...
				for i := 0; i < p.imageWidth/16; i++ {
					for k := 0; k < 16; k++ {
						section[k] = world[modulo((n*(p.imageHeight/realworker+1))+y, p.imageHeight)][i*16+k]
					}
					chans[n] <- section
				}
			}
			...
			for y := -1; y <= rows; y++ {
				...
				for i := 0; i < p.imageWidth/16; i++ {
					for k := 0; k < 16; k++ {
						section[k] = world[modulo((n*(p.imageHeight/realworker)+remains)+y, p.imageHeight)][i*16+k]
					}
					chans[n] <- section
				}
			}
...
func worker(p golParams, io chan [16]byte, halosaversup [maxthreads]chan [16]byte, halosaversdown [maxthreads]chan [16]byte, turntoworker chan int, turntodis chan int, z int) {
	var localtotalalive int
	running := true
	remains := p.imageHeight % realworker
	worldslice := make([][]byte, ((p.imageHeight / realworker) + 3))
	for i := range worldslice {
		worldslice[i] = make([]byte, p.imageWidth)
	}
	worldupdator := make([][]byte, ((p.imageHeight / realworker) + 3))
...
func distributor(p golParams, d distributorChans, alive chan []cell, chans [maxthreads]chan [16]byte, turntoworker [maxthreads]chan int, turntodis [maxthreads]chan int) {
	keypress := make(chan rune, 5)
	var paused bool
	turn := 0
	rows := p.imageHeight / realworker
	remains := p.imageHeight % realworker
