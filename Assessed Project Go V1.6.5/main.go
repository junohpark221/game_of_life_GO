...
func gameOfLife(p golParams, keyChan <-chan rune) []cell {
	...
	var chans [maxthreads]chan [16]byte
	for i := 0; i < p.threads; i++ {
		chans[i] = make(chan [16]byte)
	}
	var halosaversup [maxthreads]chan [16]byte
	for i := 0; i < p.threads; i++ {
		halosaversup[i] = make(chan [16]byte, p.imageWidth/16)
	}
	var halosaversdown [maxthreads]chan [16]byte
	for i := 0; i < p.threads; i++ {
		halosaversdown[i] = make(chan [16]byte, p.imageWidth/16)
	}
	var turntoworker [maxthreads]chan int
	for i := 0; i < p.threads; i++ {
		turntoworker[i] = make(chan int, 1)
	}
	var turntodis [maxthreads]chan int
	for i := 0; i < p.threads; i++ {
		turntodis[i] = make(chan int, 1)
	}
	if p.threads > p.imageHeight {
		realworker = p.imageHeight
	} else {
		realworker = p.threads
	}
	...
	for i := 0; i < realworker; i++ {
		go worker(p, chans[i], halosaversup, halosaversdown, turntoworker[i], turntodis[i], i)
	}
	go ticker()
	go distributor(p, dChans, aliveCells, chans, turntoworker, turntodis)
