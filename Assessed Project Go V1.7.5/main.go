...
func gameOfLife(p golParams, keyChan <-chan rune) []cell {
	...
	var halosaversup [maxthreads]chan []byte
	for i := 0; i < p.threads; i++ {
		halosaversup[i] = make(chan []byte, p.imageWidth/16)
	}
	var halosaversdown [maxthreads]chan []byte
	for i := 0; i < p.threads; i++ {
		halosaversdown[i] = make(chan []byte, p.imageWidth/16)
	}
