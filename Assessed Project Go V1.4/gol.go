...
func merger(p golParams, world [][]byte, chans [maxthreads]chan [16]byte, newimageheight int) {
...
func updator(worldslice [][]byte, worldupdator [][]byte, p golParams, newimageheight int, io chan [16]byte) {
	for y := 1; y < ((newimageheight / p.threads) + 1); y++ {
		for x := 0; x < p.imageWidth; x++ {
...
func worker(p golParams, io chan [16]byte, z int) {
	...
	for {
		localtotalalive = importer(worldslice, p, newimageheight, localtotalalive, io)
		localtotalalive = gollogic(worldslice, worldupdator, p, newimageheight, padding, localtotalalive)
		updator(worldslice, worldupdator, p, newimageheight, io)
...
func distributor(p golParams, d distributorChans, alive chan []cell, chans [maxthreads]chan [16]byte) {
	...
	for turn < p.turns {
		...
		default:
			...
			merger(p, world, chans, newimageheight)
		}
		turn++
	}
	// Stop ticker thread.
