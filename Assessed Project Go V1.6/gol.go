...
func merger(p golParams, world [][]byte, chans [maxthreads]chan [16]byte, newimageheight int, notifiers [maxthreads]chan bool) {
...
func importer(worldslice [][]byte, p golParams, newimageheight int, localtotalalive int, io chan [16]byte) int {
	localtotalalive = 0
...
func sender(worldslice [][]byte, p golParams, newimageheight int, io chan [16]byte) {
	for y := 1; y < ((newimageheight / p.threads) + 1); y++ {
		for x := 0; x < p.imageWidth/16; x++ {
			var section [16]byte
			for k := 0; k < 16; k++ {
...
func exporter(world [][]byte, p golParams, newimageheight int, padding int, chans [maxthreads]chan [16]byte) {
	for n := 0; n < p.threads; n++ {
			...
			for i := 0; i < p.imageWidth/16; i++ {
				for k := 0; k < 16; k++ {
					if (n+1)*(newimageheight/p.threads) <= padding {
						if n == 0 && y == -1 {
							section[k] = world[0][i*16+k]
						} else {
							section[k] = world[modulo(n*(newimageheight/p.threads)+y, newimageheight)][i*16+k]
						}
					} else if padding < (n+1)*(newimageheight/p.threads) && (n+1)*(newimageheight/p.threads) <= (padding+newimageheight/p.threads) && y == -1 {
						section[k] = world[newimageheight-1][i*16+k]
					} else if n == p.threads-1 && y == newimageheight/p.threads {
						section[k] = world[padding][i*16+k]
					} else {
						section[k] = world[modulo(n*(newimageheight/p.threads)+y, newimageheight)][i*16+k]
					}
				}
...
func haloex(worldslice [][]byte, p golParams, commsup [maxthreads](chan [16]byte), commsdown [maxthreads](chan [16]byte), padding int, z int, newimageheight int) {
	...
	if z == firstrealworker {
		for x := 0; x < p.imageWidth/16; x++ {
			var section [16]byte
			for k := 0; k < 16; k++ {
				section[k] = worldslice[firstindex+1][x*16+k]
			}
	...
	for x := 0; x < p.imageWidth/16; x++ {
		var section [16]byte
		for k := 0; k < 16; k++ {
			section[k] = worldslice[newimageheight/p.threads][x*16+k]
		}
		if z == p.threads-1 {
...
func worker(p golParams, chans [maxthreads](chan [16]byte), commsup [maxthreads](chan [16]byte), commsdown [maxthreads](chan [16]byte), notifier chan bool, z int) {
...
func distributor(p golParams, d distributorChans, alive chan []cell, chans [maxthreads]chan [16]byte, notifiers [maxthreads]chan bool) {
