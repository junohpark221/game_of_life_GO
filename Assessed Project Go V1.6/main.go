...
func gameOfLife(p golParams, keyChan <-chan rune) []cell {
	...
	var commsup [maxthreads](chan [16]byte)
	for i := 0; i < p.threads; i++ {
		commsup[i] = make(chan [16]byte, p.imageWidth/16)
	}
	var commsdown [maxthreads](chan [16]byte)
	for i := 0; i < p.threads; i++ {
		commsdown[i] = make(chan [16]byte, p.imageWidth/16)
	}
